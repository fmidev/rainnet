            ...
            for id in metric_exp_ids:
                ...
                for i,n in enumerate(name_now):
                    for lt, val in enumerate(npy_now[i]):
                        s = pd.Series([metric, n, method, (lt+1)*5, val], index=scoredf.columns)
    ...
    if method == "ALL":
        ...
        for method_i, (val, names) in scores.items():
            for i, n in enumerate(names):
                ...
                plot_func(
                    lt, val[i, plot_lt_indices], **method_plot_params[method_i], **plot_kwargs
                )
    else:
        ...
        for i, n in enumerate(names):
            ...
            plot_func = match_axis_type(axis_type, plot[n][1])
            plot_func(lt, val[i, plot_lt_indices], **method_plot_params[method], **plot_kwargs)

    for _, ax in plot.values():
        ax.legend()
        ax.grid()
        ax.set_xticks(np.arange(*xticks))
        ax.set_ylim(xlim_val)
